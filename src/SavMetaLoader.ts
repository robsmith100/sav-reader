...
import { DictionaryTerminationRecord } from "./records/DictionaryTerminationRecord";
...
import { ValueLabelRecord } from "./records/ValueLabelRecord";
import { VariableRecord } from "./records/VariableRecord";
...
export class SavMetaLoader{
    ...
    constructor(chunkReader: ChunkReader){
    ...
    async load(){
        ...
        let recent_stringvar: SysVar = null; // keep most recent string for easy linking of string continuation vars
        let done = false;
        do{
        ...
        this.meta.valueLabels = this.meta.valueLabels.map(set => {
            var set2 = {...set};
            set2.appliesTo = set2.appliesTo.map((shortname: string) => this.meta.sysvars.find(sysvar => sysvar.shortName == shortname).name);
            return set2;
        });
